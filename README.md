...
- DDL 页面一键 AI 分析当前任务列表，自动梳理任务优先级与时间安排
- 结合课表、任务截止时间、优先级等上下文，生成结构化分析与执行建议
- 分析过程后台持续运行，切换页面不中断，完成后自动展示结果
- 假期状态下自动切换为休息提醒与假期任务预警逻辑
...
- 支持透明度50%-100%调整
- 支持课程卡片模糊蒙版
- 支持视频壁纸，可将本地视频设为课表背景，播放时可伴随音频
- 视频壁纸支持播放控制（播放/暂停）

### 桌面小组件

- 提供 2x2、4x2、4x4 三种尺寸的安卓桌面小组件
- 2x2：紧凑展示下一节课信息与倒计时，课程结束后显示"今日课程已结束"
- 4x2：列表展示今日剩余课程，支持"剩余 N 节"指示
- 4x4：完整今日课程列表，今日课程结束后自动展示明日课程
...
- One-tap AI analysis of your current task list on the DDL page, automatically prioritizing tasks and scheduling
- Generates structured analysis and actionable suggestions based on timetable, deadlines, and priorities
- Analysis runs in the background and survives page switches, displaying results when complete
- Automatically switches to rest reminders and holiday task alerts when in holiday mode
...
- Personalize the timetable page with your own wallpaper
- Adjustable transparency from 50% to 100%
- Frosted glass blur mask for course cards
- Video wallpaper support — set a local video as the timetable background with audio playback
- Video wallpaper supports playback controls (play/pause)

### Home Screen Widgets

- Three Android home screen widget sizes: 2x2, 4x2, and 4x4
- 2x2: Compact view of the next class with countdown; shows "Today's classes finished" when done
- 4x2: List of remaining classes today with "N remaining" indicator
- 4x4: Full today's class list, automatically showing tomorrow's classes when today's are finished
