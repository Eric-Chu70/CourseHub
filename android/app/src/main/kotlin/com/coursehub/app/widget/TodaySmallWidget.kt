...
class TodaySmallWidget : GlanceAppWidget() {
            ...
            Box(
                ...
                Box(
                    ...
                    Row(
                        ...
                        Text(
                            text = if (data.isHoliday) "假期中" else data.label,
                            modifier = GlanceModifier.fillMaxWidth(),
                            style = TextStyle(
                                color = WidgetTheme.hintProvider(),
                                fontSize = 10.sp,
    ...
    private fun BottomDotsRow(data: WidgetData.TodayData, current: WidgetData.Course) {
        // 正在上课时，剩余课程不含当前课；下节课状态时，剩余课程含下节课
        val remainingCourses = if (current.isCurrent) {
            data.courses.filter { it.periodStart > current.periodStart }
        } else {
            data.courses.filter { it.periodStart >= current.periodStart }
        }
